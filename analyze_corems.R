...
# #########################################################################################
# #
# # Unit test communities
# #
# #########################################################################################
# 
# unitTest <- function(input,outName) {
#   # Expects "input" table with following columns (n x 5):
#   # "Gene1","Gene2","Community.ID","Community.Density","Community.Weighted.Density"
#   # outName should be the name of the directory you want to create to store unit test 
#   data<-read.table(input)
#   colnames(data) <- 
#     c("Gene1","Gene2","Community.ID","Community.Density","Community.Weighted.Density")
#   # Make dir to place files
#   system(paste("mkdir",outName,sep=" "))
#   dir <- paste("~/",outName,"/",sep="")
#   # Threshold data. Require density of regulons be > 0
#   regulons<-data[data[,5]>0,]
#   # Plot #communities as function of density threshold
#   plotSS <- function(data=data, inc=0.001) {
#     d <- c()
#     d.weighted <- c()
#     for (i in seq(0,1,inc)) {
#     d <- append(d,sum(length(unique(
#        data[data[,4]>=i,3]))))
#       d.weighted <- append(d.weighted,sum(length(unique(
#         data[data[,5]>=i,3]))))
#     }
#     pdf(file=paste(dir,"summary_statistics.pdf",sep="")
#     par(mfrow=c(1,3))
#     # Plot # communities as function of density
#      plot(seq(0,1,inc),d,type="p",col=1,ylim=c(1,max(d)),ylab="# Communities",main="Density of communities",
#           xlab= "Density cutoff",log="y")
#      lines(seq(0,1,inc),d.weighted,type="p",col=2,log="y",pch=2)
#      legend("topright",legend=c("Density","Weighted Density"),col=c(1,2),pch=c(1,2),cex=1)
#      mtext(paste(d[2],"communities with density > 0"),side=3,line = -8)
#      mtext(paste(length(unique(c(as.character(regulons[,1]),as.character(regulons[,2])))),"genes represented"),side=3,line = -9)
#     #############################
#     # Plot genes per community
#     # and
#     # Communities per gene
#     #############################
#     # Get all regulons
#     regulons.ids <- unique(regulons[,3])
#     regulons.genes.per <- c()
#     for (i in regulons.ids) {
#       tmp.id <- regulons[which(regulons[,3]==i),]
#       regulons.genes.per <- append(regulons.genes.per,
#                                length(unique(c(as.character(tmp.id[,1]),as.character(tmp.id[,2])))))
#            }
#     hist(regulons.genes.per,main="# Genes Per Community",xlab="# Genes",breaks=100,)
#     # Communities per gene
#     regulons.genes<-unique(c(as.character(regulons[,1]),as.character(regulons[,2])))
#     regulons.per.gene <- c()
#     for (i in regulons.genes) {
#       tmp.id <- unique(c(regulons[which(regulons[,1]==i),3],regulons[which(regulons[,2]==i),3]))
#       regulons.per.gene <- append(regulons.per.gene,
#                                length(tmp.id))
#            }
#     hist(regulons.per.gene,main="# Communities Per Gene",xlab="# Communities",breaks=100)
#     dev.off()
#     }
#   plotD()
#   
...
# ##########################
# #
# #  Old code of limited utility
# #
# #
# ##########################
# # Analyze community #40529
# # Randomly selected
# tmp<-gBg_cutoff_0p01.communities_0.7400[gBg_cutoff_0p01.communities_0.7400[,5]>.0001,]
# tmp.40529<-tmp[which(tmp[,3]==40529),]
# tmp.40529.genes<-unique(c(as.character(tmp.40529[,1]),as.character(tmp.40529[,2])))
# tmp.40529.conditions <- 
#   egrin2.agglom(src=tmp.40529.genes, srcType="gene", targetType="condition",path="bicluster")
# tmp.40529.conditions.sig<-tmp.40529.conditions[tmp.40529.conditions[,3]<.05,]
# tmp.40529.genes.expression.cor <- 
#   cor(t(rats[tmp.40529.genes,rownames(tmp.40529.conditions.sig)]))
# save(tmp.40529.genes,tmp.40529.conditions,file="tmp.40529.genes.RData")
# library(gplots)
# rats<-e$ratios[[1]]
# heatmap.2(tmp.40529.genes.expression.cor,dendrogram="row",symm=T,trace="none")
# 
# # Analyze # communities as a function of cutoff
# load("~/R/egrin2/gBg_antoine/gBg_cutoff_0p01.communities_0.7400.RData")
# inc <- 0.0001
# d <- c()
# d.weighted <- c()
# for (i in seq(0,1,inc)) {
#   d <- append(d,sum(length(unique(
#     gBg_cutoff_0p01.communities_0.7400[gBg_cutoff_0p01.communities_0.7400[,4]>=i,3]))))
#   d.weighted <- append(d.weighted,sum(length(unique(
#     gBg_cutoff_0p01.communities_0.7400[gBg_cutoff_0p01.communities_0.7400[,5]>=i,3]))))
# }
# pdf(file="~/R/egrin2/gBg_antoine/communities_denistycutoff_plot.pdf")
#   plot(seq(0,1,inc),d,type="p",col=1,ylim=c(1,max(d)),ylab="# Communities",main="Limiting # of communities",
#       xlab= "Density cutoff",log="y")
#   lines(seq(0,1,inc),d.weighted,type="p",col=2,log="y",pch=2)
#   legend("topright",legend=c("Density","Weighted Density"),col=c(1,2),pch=c(1,2),cex=1.5)
# dev.off()
# 
# # Analyze linkage of communities
# 
# # Threshold communities
# regulons<-gBg_cutoff_0p01.communities_0.7400[gBg_cutoff_0p01.communities_0.7400[,5]>=.0001,]
# save(gBg_cutoff_0p01.communities_0.7400,regulons,file="gBg_cutoff_0p01.communities_0.7400.RData")
# # Portion of genome represented
# # 798 genes out of 2261 (without threshold), 35.29%
# regulons.genes<-unique(c(as.character(regulons[,1]),as.character(regulons[,2])))
# # Genes per community
# # Get all regulons
# regulons.ids <- unique(regulons[,3])
...
#                                length(unique(c(as.character(tmp.id[,1]),as.character(tmp.id[,2])))))
#            }
# pdf(file="~/R/egrin2/gBg_antoine/genes_per_regulon.pdf")
#   hist(regulons.genes.per,main="# Genes Per Community",xlab="# Genes",breaks=100,)
# dev.off()
# # Communities per gene
# regulons.per.gene <- c()
...
#                                length(tmp.id))
#            }
# pdf(file="~/R/egrin2/gBg_antoine/regulons_per_gene.pdf")
#   hist(regulons.per.gene,main="# Communities Per Gene",xlab="# Communities",breaks=100)
# dev.off()
# 
# # Similarity of regulons
# regulon.similarity <- matrix(nrow=length(regulons.ids),ncol=length(regulons.ids),
#                              dimnames=list(regulons.ids,regulons.ids))
# for (m in 1:length(regulons.ids)) {
#   print(m)
#   i <- regulons.ids[m]
#   tmp.i <- regulons[which(regulons[,3]==i),]
#   i.genes <- unique(c(as.character(tmp.i[,1]),as.character(tmp.i[,2])))
#   for (n in 1:length(regulons.ids)) {  
#     j <- regulons.ids[n]
#     tmp.j <- regulons[which(regulons[,3]==j),]
#     j.genes <- unique(c(as.character(tmp.j[,1]),as.character(tmp.j[,2])))
#     overlap.i.j <- sum(i.genes%in%j.genes)/(min(c(length(i.genes),length(j.genes))))
#     regulon.similarity[which(rownames(regulon.similarity)==i),which(rownames(regulon.similarity)==j)] <- overlap.i.j
#   }
# }
# #This takes ages to compute. Save it!
# save(regulon.similarity,file="gBg_cutoff_0p01.communities_0.7400_regulon_similarity.RData")
# library(gplots)
# heatmap.2(regulon.similarity,dendrogram="row",symm=T,trace="none")
# 
# # Arginine regulation genes 
# # VNG0970G,VNG0971G,VNG0973G,VNG0974G,VNG1760G,VNG1762G,VNG6313G,VNG6315G,VNG6316G,VNG6317G,VNG6318G
# # ???? VNG2366C, VNG0796G, VNG5123H, VNG2162C/3H,VNG1086C,VNG0804C
# load("~/R/egrin2/arginine_catabolism/arginineGenes.RData")
# arginine.genes <- append(arginine.genes,"VNG1762G")
# rats<-e$ratios[[1]]
# arginine.genes.regulons <- lapply(arginine.genes,function(i) {
#   i<-unique(regulons[c(grep(i,regulons[,1]),grep(i,regulons[,2])),3])
#   })
# names(arginine.genes.regulons) <- arginine.genes
# save(arginine.genes,arginine.genes.139587,arginine.genes.9450,arginine.genes.9450.9645,
#      file="~/R/egrin2/arginine_catabolism/expanded_argining_genes_regulons.RData")
# 
# # Params for heatmap
# pairs.breaks <- seq(-1, 1, by=0.05)
# mycol <- colorpanel(n=40,low="blue",mid="white",high="red")
# 
# ##
# ## Regulon 139587
# ##
# arginine.genes.139587.conditions <- 
#   egrin2.agglom(src=arginine.genes.139587, srcType="gene", 
#                 targetType="condition",path="bicluster")
# arginine.genes.139587.conditions.sig <-
#   arginine.genes.139587.conditions[arginine.genes.139587.conditions[,3]<.05,]
# arginine.genes.139587.expression.cor <- 
#   cor(t(rats[arginine.genes.139587,rownames(arginine.genes.139587.conditions.sig)]))
# heatmap.2(arginine.genes.139587.expression.cor,dendrogram="row",symm=T,
#           trace="none",symbreaks=pairs.breaks, col = mycol)
# 
# ##
# ## Regulon 9450
# ##
# arginine.genes.9450.9645.conditions <- 
#   egrin2.agglom(src=arginine.genes.9450.9645, srcType="gene", 
#                 targetType="condition",path="bicluster")
# arginine.genes.9450.9645.motifs <- 
#   egrin2.agglom(src=arginine.genes.9450.9645, srcType="gene", 
#                 targetType="motif.cluster",path="motif")
# arginine.genes.9450.9645.conditions.sig <-
#   arginine.genes.9450.9645.conditions[arginine.genes.9450.9645.conditions[,3]<.05,]
# arginine.genes.9450.9645.expression.cor <- 
#   cor(t(rats[arginine.genes.9450.9645,rownames(arginine.genes.9450.9645.conditions.sig)]))
# arginine.genes.9450.9645.expression.cor.allconds <- 
#   cor(t(rats[arginine.genes.9450.9645,]))
# heatmap.2(arginine.genes.9450.9645.expression.cor,dendrogram="row",symm=T,
#           trace="none",symbreaks=pairs.breaks, col = mycol)
# heatmap.2(arginine.genes.9450.9645.expression.cor.allconds,dendrogram="row",symm=T,
#           trace="none",breaks=pairs.breaks, col = mycol)
# 
# ##
# ## All regulons containing arginine pathway genes
# ##